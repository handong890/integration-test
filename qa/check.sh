...
echo -e "\n `date`-----------------Kibana log-------------------------------------------"
# try 10 times, 2 seconds apart
for i in `seq 1 20`; do echo "${i} `date` grep kibana log for 'green'" && sleep 4 & grep '"plugin:elasticsearch@.*","info"],"pid":.*,"state":"green"' /var/log/kibana/kibana.stdout && break; done
...
echo " `date`-- http://elastic:changeme@localhost:9200/_cat/indices"
